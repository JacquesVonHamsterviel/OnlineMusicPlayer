...
class Meting
{
    ...
    public function pic($id, $size = 300)
    {
        switch ($this->server) {
            case 'netease':
            $url = 'https://fetch.kkk.plus/'.base64_encode('https://p3.music.126.net/'.$this->netease_encryptId($id).'/'.$id.'.jpg?param='.$size.'y'.$size);;
            break;
            case 'tencent':
            $url = 'https://y.gtimg.cn/music/photo_new/T002R'.$size.'x'.$size.'M000'.$id.'.jpg?max_age=2592000';
            $url = 'https://fetch.kkk.plus/'.base64_encode($url);
            break;
            case 'xiami':
            $format = $this->format;
    ...
    private function netease_url($result)
    {
        ...
        if (isset($data['data'][0]['url'])) {
            $proxy_url='https://fetch.kkk.plus/'.base64_encode(str_replace('http://', 'https://', $data['data'][0]['url']));
            $url = array(
                'url' =>  $proxy_url,
